...
class ActionRecognitionDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        # Define transforms
        train_transform = (
            self._get_train_transforms() if self.augment else self._get_val_transforms()
        )
        ...
        if self.dataset_type == "frame_image":
            ...
            self.val_dataset = FrameImageDataset(
                root_dir=self.root_dir, split="val", transform=val_transform
            )
            self.test_dataset = FrameImageDataset(
                root_dir=self.root_dir, split="test", transform=val_transform
            )
...
if __name__ == "__main__":
    ...
    )  # Expected: (batch, channels, time, height, width)
